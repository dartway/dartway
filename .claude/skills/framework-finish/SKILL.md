...
**Шестое зеркало — за пределами репозитория.** Изменил `packages/dartway_studio_bridge` — у контракта две стороны, и вторая живёт в отдельном репозитории (`dartway/dartway_studio`). Соседства папок больше нет, поэтому проверка держится только на этом пункте:
