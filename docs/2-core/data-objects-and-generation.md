...
(`dart run dartway_cli:dartway generate` reports a class that does). A project class extends one of three kinds:
...
```bash
cd <project>_flutter
dart run dartway_cli:dartway generate          # writes parts and registries, removes stale parts
dart run dartway_cli:dartway generate --check  # writes nothing; exits 1 when anything is out of date. For CI.
```
...
**Never edit a generated file.** Each starts with `// GENERATED BY dart run dartway_cli:dartway generate. DO NOT EDIT.`;
the next run overwrites it, and `dart run dartway_cli:dartway check` fails with `generatedCodeStale` while it differs from
