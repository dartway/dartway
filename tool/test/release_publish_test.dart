...
void main() {
  ...
  test(
    'a short-window rate limit is retried and succeeds, without exhausting '
    'the retry budget',
    () async {
  ...
  test(
    'a short-window rate limit that never clears is deferred once the retry '
    'budget is spent, not retried forever',
    () async {
      final publisher = _ScriptedPublisher()
        ..queue('a', List.generate(
          10,
  ...
  test(
    'a daily rate limit defers the package and every plan entry depending '
    'on it, and still publishes the rest of the order',
    () async {
      final publisher = _ScriptedPublisher()
      ...
      final plan = [
        ...
        _Unit('dartway_auth_providers_server', [
      ...
      expect(
        ...
        containsAll(['dartway_auth_providers_shared', 'dartway_auth_providers_server']),
      );
      expect(
      ...
      expect(
        ...
        reason: 'a dependent is deferred for depending on the blocked '
  ...
  test(
    'a package failing to become visible stops the run, even though '
    'publishing it succeeded',
    () async {
      final publisher = _ScriptedPublisher()
        ..queue('a', [const PublishOk()])
        ..queue('b', [const PublishOk()]);
