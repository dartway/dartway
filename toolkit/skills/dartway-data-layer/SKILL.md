...
## 8. Real-time между пользователями — общий канал (важно, легко ошибиться)

**Чего НЕТ из коробки:** `saveModel` возвращает обновления **только тому, кто сохранял** — его `watchModelList` обновляется из ответа. Каждый клиент по умолчанию слушает лишь свой личный канал. Поэтому «клиент А записался — у клиента Б на экране само обновилось» **не работает автоматически.** Это самая частая ложная ожидалка: `watchModelList` реактивен к *своим* правкам, но не вещает чужие.

Чтобы обновление долетело до других, заведи **именованный канал** и слей в него апдейт — обе стороны:

```dart
// СЕРВЕР — в хуке CRUD-конфига (напр. afterSaveTransaction), после пересчёта:
await session.messages.postMessage(
  'slotAvailabilityChannel',                       // имя канала — общее для обеих сторон
  DwUpdatesTransport(
...
```dart
// FLUTTER — оберни экран/список в подписку на тот же канал:
DwChannelSubscriptionWidget(
  channel: 'slotAvailabilityChannel',
  child: scheduleList,   // внутри — обычный ref.watchModelList<ClubSession>()
)
```

Пришедший в канал `ClubSession` роутится по типу в **любой** `watchModelList<ClubSession>()` у подписанных клиентов — список перерисовывается сам, без кода обновления. Имя канала на сервере и во Flutter должно совпадать буквально.

**Личный апдгейт одному пользователю** (а не всем) — `session.sendUpdatesToUser(userProfileId, updatedModels: [...])`: клиент слушает свой канал всегда, отдельная подписка не нужна. Годится, когда меняется что-то приватное конкретного юзера (его бронь отменили), а не общий счётчик.

Выбор: **общее и публичное** (свободные места, лента) → именованный канал + `DwChannelSubscriptionWidget`; **приватное одному** → `sendUpdatesToUser`.
