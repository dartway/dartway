...
3. **Measure the distance to the framework.** `resolved-ref` in `pubspec.lock` against the DartWay monorepo's current `master`, and what landed there since. Two things follow: a workaround in this project may have become a duplicate of something the framework now does, and the installed harness may be teaching rules the current framework no longer holds (the harness channel must match the framework channel).
4. **Read the journals**: `dev_notes.md` and `dartway_notes.md` at the project root. Open entries are context, and an entry that is already fixed should be closed in this run.
...
- **hacks**: workarounds, `TODO`/`HACK`/`FIXME`, temporary patches, commented-out code, CRUD violations (an arbitrary endpoint instead of `saveModel`/`watchModel`);
