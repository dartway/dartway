...
1. **Run every gate the project has** and record the result of each. A failure here is a fact, not an opinion.
   ...
   - the tests: `dart test` in `__SHARED_PKG__`, `flutter test` in `__FLUTTER_PKG__`, and `dart run dartway_cli:dartway test` for the server — it starts a Postgres and a MinIO of its own for the run, so it needs Docker and nothing else.
