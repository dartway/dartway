...
- **Данные (data-layer):** чтение — `ref.watchModel`/`readModel`/`watchMaybeModel`/`readMaybeModel`/`watchModelList`; запись — статические `DwRepository.saveModel`/`DwRepository.deleteModel` (`ref.saveModel` не существует). Списки — `dwBuildListAsync(loadingItemsCount:)`; локальный фильтр — `frontendFilter`. Контракт — `dartway-data-layer`, создание фичи — `dartway-feature-scaffold`.
