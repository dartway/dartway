...
- **Every project is localized, and user-visible text is never written in code.** This is a requirement on the project, not a report on how it began. **Check that the wiring is actually there, and if it is not, putting it there is the first thing fixed — not something to live with.** What has to be present: `flutter_localizations` and `generate: true` in the Flutter pubspec, `l10n.yaml` and `lib/l10n/*.arb` with its generated output committed beside them, `appLocaleProvider` (system locale by default, switchable at runtime and by Studio over the bridge), `context.l10n` in widgets and `appL10n` for code outside the tree — an error toast, a dialog raised from a handler.

  **The law reaches as far as the app does, and no further.** Text composed on the *server* — a push body, a transactional email — is outside it: the queue stores a finished `title`/`body` and the module sends it as it is, there is no `appL10n` there and cannot be. That text is its own subject and has no rule yet, which is a gap named rather than covered: naming notification bodies here, as this line used to, promised a coverage that did not exist and left a project reading the law honestly with no way to comply. Until that rule exists, a project sending server-composed text in more than one language is deciding for itself how — and should say so somewhere its next reader will look.

  A project created by `dartway create` arrives with all of it, and the skeleton is where to read what each piece looks like. A project that reached this methodology by another road may have none of it, and **nothing else will say so**: the compiler is happy, the tests are green, and the app looks finished. Left to itself it surfaces as one item of an otherwise consistent menu turning up in a different language — a human noticing a screenshot, which is not a check. So `dartway check` reports it as `l10nNotWired`, an error, naming which of the four pieces is absent.
...
- **Navigation:** the DartWay Router — enum routes, enum parameters, transitions through context extensions (`context.goNamed`/`pushTo`/`replaceWith`, not `router.go()`), guards centralized. One exception, and it is a fact rather than a preference: a transition nobody started from the tree — a tapped notification, a deep link — has no context to go through, and takes the navigation function from the router in one seam per application, in `core/`. Skill — `dartway-navigation`.
