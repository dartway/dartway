...
import 'showcase_persona.dart';

part 'showcase_user_switcher.g.dart';
...
class ShowcaseUserSwitcher extends _$ShowcaseUserSwitcher {
  @override
  bool build() => false;

  Future<void> switchTo(ShowcasePersona persona) async {
    if (state) return;
    state = true;
    try {
      ...
      final verification = await DwRepository.saveModel(
        DwAuthVerification(
          ...
          verificationCode: showcaseDevOtpCode,
        ),
        apiGroupOverride: DwCoreConst.dartwayInternalApi,
      );
