...
- **Язык:** **всё, что может оказаться на людях, — английский.** Доки, README пакетов, строки ошибок, комментарии в коде пакетов — и **`toolkit/` тоже**: он едет в `.claude/` каждого проекта на фреймворке и лежит в публичном репозитории, то есть уже на людях. Русский остаётся только там, куда посторонний не попадает: `project/` (приватный репо) и наши обсуждения. На каком языке проект пишет **свои** паспорта фич и доки — его дело, это настройка проекта, а не наша.
...
Бэклог доработок — в Notion, база **Deliverables**. Координаты доски, поля и порядок заведения карточек лежат в `project/NOTION_BOARD.md`: `project/` — приватный репозиторий, а этот файл теперь публичный, и внутренние id доски в нём не место.
