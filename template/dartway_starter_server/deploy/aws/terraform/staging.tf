resource "aws_launch_configuration" "staging" {
  ...
  name_prefix = "${var.dartway_starter}-staging-"
...
resource "aws_autoscaling_group" "staging" {
  ...
  tag {
    key                 = "Name"
    value               = "${var.dartway_starter}-serverpod-staging"
    propagate_at_launch = true
  }

  tag {
    key                 = "CodeDeploy"
    value               = "${var.dartway_starter}-staging"
    propagate_at_launch = true
  }
}
...
resource "aws_codedeploy_deployment_group" "staging" {
  ...
  deployment_group_name = "${var.dartway_starter}-staging-group"
