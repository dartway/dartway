...
resource "aws_lb" "serverpod_staging" {
  ...
  name               = "${var.dartway_starter}-serverpod-staging"
  internal           = false
...
resource "aws_lb_target_group" "api_staging" {
  ...
  name     = "${var.dartway_starter}-api-staging"
  port     = 8080
  protocol = "HTTP"
...
resource "aws_lb_target_group" "insights_staging" {
  ...
  name     = "${var.dartway_starter}-insights-staging"
  port     = 8081
  protocol = "HTTP"
  vpc_id   = module.vpc.vpc_id
}
...
resource "aws_lb_target_group" "web_staging" {
  ...
  name     = "${var.dartway_starter}-web-staging"
  port     = 8082
  protocol = "HTTP"
  vpc_id   = module.vpc.vpc_id
}
