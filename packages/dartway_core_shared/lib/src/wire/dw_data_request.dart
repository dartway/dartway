...
/// from [positionOf] (`DwWindowCursor`), so rows sharing a sort value are
/// neither lost nor repeated.
///
/// [positionOf] is the one definition of the sequence's order: the server
/// builds cursors and checks a client's cursors with it, and the client
/// decides with it where an object arriving live belongs. [S] is the sort
/// value (`int`, `String` or `DateTime`), [I] the id (`int` or `String`).
///
/// ```dart
/// final class ListChat extends DwWindowRequest<ChatMessage, DateTime, int>
///     with _$ListChat {
///   const ListChat({required this.roomId}) : super(pageSize: 30);
///   final int roomId;
///   @override
...
/// replaced; a new matching one is inserted where [positionOf] puts it when
/// that is inside the loaded range: newer than the newest row only while the
/// window shows the newest rows (`hasNewer` false — otherwise the window
/// counts it as unseen), older than the oldest row only while it shows the
/// oldest (`hasOlder` false — otherwise it appears on loading older rows).
/// One that stops matching, or a deletion, is removed.
abstract class DwWindowRequest<
...
>
  ...
  /// [item]'s place in the sequence: its sort value and its id. A pure
  /// function of the object — the same column the server's handler orders
  /// by, or cursors and live inserts disagree with the rows it reads.
  DwWindowPosition<S, I> positionOf(T item);

  /// The cursor string of [item]'s position.
  String cursorOf(T item) {
    final (:sortValue, :id) = positionOf(item);
    return DwWindowCursor.encode(sortValue, id);
  }

  /// Compares the positions of [a] and [b]: positive when [a] is newer.
  int compareItems(T a, T b) =>
      DwWindowCursor.comparePositions(positionOf(a), positionOf(b));
