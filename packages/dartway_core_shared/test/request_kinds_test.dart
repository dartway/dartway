...
void main() {
  ...
  group('window order (positionOf)', () {
    ClubBooking at(int id, int hour) => ClubBooking(
      id: id,
      status: BookingStatus.booked,
      startsAt: DateTime.utc(2026, 9, 14, hour),
    );

    test('one definition gives the cursor and the comparison', () {
      const request = BookingHistory();
      final row = at(7, 10);
      expect(
        DwWindowCursor.decode(request.cursorOf(row)),
        DwWindowCursor(DateTime.utc(2026, 9, 14, 10), 7),
      );
      expect(request.compareItems(at(1, 11), at(9, 10)), greaterThan(0));
      expect(request.compareItems(at(9, 10), at(1, 11)), lessThan(0));
    });

    test('ties on the sort value are ordered by id', () {
      const request = BookingHistory();
      expect(request.compareItems(at(8, 10), at(7, 10)), greaterThan(0));
      expect(request.compareItems(at(7, 10), at(7, 10)), 0);
      final sorted = [at(2, 10), at(1, 12), at(3, 10), at(4, 9)]
        ..sort((a, b) => request.compareItems(b, a));
      expect(sorted.map((b) => b.id), [1, 3, 2, 4], reason: 'newest first');
    });
  });
