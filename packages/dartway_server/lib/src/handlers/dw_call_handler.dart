...
/// A position in a window's sequence, typed as the handler declared it.
typedef DwWindowPosition<S, I> = ({S sortValue, I id});
...
sealed class DwCallHandler {
  ...
  ///
  /// [positionOf] names a row's place in the sequence: its sort value [S]
  /// (`int`, `String` or `DateTime`) and its id [I] (`int` or `String`) —
  /// what cursors are built from, and what a cursor from the client must
  /// decode to, or the call is malformed. [handle] reads one direction (see
  ...
  >({
    required DwAccessRule access,
    int? maxBodyBytes,
    required DwWindowPosition<S, I> Function(T item) positionOf,
    required Future<List<T>> Function(
      DwCallContext ctx,
      Q request,
    ...
    handle,
  }) => DwWindowHandler<Q, T, S, I>._(access, maxBodyBytes, positionOf, handle);
...
>
  ...
  final DwWindowPosition<S, I> Function(T item) _positionOf;
  final Future<List<T>> Function(
    DwCallContext ctx,
    Q request,
    DwWindowInput<S, I> window,
  ...
  String _cursorOf(T item) {
    final (:sortValue, :id) = _positionOf(item);
  ...
  Future<Object?> run(ctx, request, prepared) async {
    ...
    return typed.encodeResult(
      DwWindowResult<T>(
        items,
        olderCursor: hasOlder ? _cursorOf(items.last) : null,
        newerCursor: hasNewer ? _cursorOf(items.first) : null,
      ),
      ctx.protocol,
    );
  }
