...
The Flutter half of the [DartWay](https://dartway.dev) core family — the one framework package a Flutter app imports. It re-exports the shared contract (`dartway_core_shared`), the client (`dartway_client`) and the router (`dartway_router`), so the app's `pubspec.yaml` names one package whose version moves the rest.

Around the data layer it is the app's skeleton:
...
The data layer (`DwFlutterCore`: `dw.request`, `dw.pages`, `dw.table`,
`dw.window`, `dw.command`) talks to a `dartway_core_server` backend; the
skeleton above has no server dependencies and works in any Flutter app.
