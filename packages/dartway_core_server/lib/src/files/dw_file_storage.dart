...
/// Where uploaded files are kept: one bucket of an S3-compatible storage
/// (AWS S3, MinIO, Yandex Object Storage, Cloudflare R2 …).
///
/// The server holds the keys and signs; clients get presigned URLs for one
/// object at a time and never see the keys. The bucket itself stays private:
/// public files are read through [publicBaseUrl], which the operator points
/// at the public purposes — a CDN in front of the bucket, or a bucket policy
/// that allows anonymous reads of those purposes' prefixes.
///
/// A browser app uploads cross-origin, so the bucket's CORS configuration
/// must allow `PUT` from the app's origin with the headers `content-type`
/// and `if-none-match`.
final class DwFileStorageConfig {
  ...
  /// Reads `ENDPOINT`, `REGION`, `BUCKET`, `ACCESS_KEY`, `SECRET_KEY`,
  /// `PUBLIC_BASE_URL` and `PATH_STYLE` under [prefix].
  ///
  /// Every missing required key and every malformed value is reported at
  /// once, as `DwDatabaseConfig.fromEnvironment` does.
  factory DwFileStorageConfig.fromEnvironment(
    ...
    final bucket = value('BUCKET', required: true);
  ...
  /// object key. `null` when no purpose is public; a public rule without it
  /// fails the server's startup.
  ...
  List<String> get problems => [
    ...
    if (!RegExp(r'^[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]$').hasMatch(bucket))
      'file storage bucket "$bucket" is not a valid bucket name',
  ...
  @override
  String toString() =>
      'DwFileStorageConfig($endpoint, bucket: $bucket, region: $region, '
      'pathStyle: $pathStyle, publicBaseUrl: $publicBaseUrl)';
}
...
enum DwFileVisibility {
  /// Read by URL, by anyone who has it (avatars, product photos). The URL is
  /// permanent and costs no call.
  public,

  /// Read through `DwGetFileLink` and `DwFileStorage.canRead` (documents,
  /// chat photos). The default everywhere a choice is made.
  private,
}
...
/// builds itself (`<purpose>/<account>/<random>.<ext>`), checks the stored
/// object by `HEAD` on `DwFinishUpload`, and records every file in
/// `dw_stored_file`. Uploads that are not finished are removed with their
/// objects by a framework job once their ticket and [uploadGrace] have
/// passed.
final class DwFileStorage {
  ...
  List<String> get problems {
    ...
    for (final rule in rules) {
      ...
      if (rule.visibility == DwFileVisibility.public &&
          config.publicBaseUrl == null) {
        problems.add(
          'upload purpose "$name" is public, and the file storage has no '
          'publicBaseUrl to serve it from',
        );
      }
    }
