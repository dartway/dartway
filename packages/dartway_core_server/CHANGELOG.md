...
- **`DwCallContext.recordProvisionalOutcome(value)`** — for a `transactional: false` command handler whose own transaction commits before its post-transaction work (a delivery, a provider call) finishes: call it from inside that transaction with what the answer will be if nothing past that point fails, and a duplicate send of the same idempotency key arriving once the transaction commits — but before the handler itself returns — replays it instead of running the handler a second time. Found on `DwRequestCode`/`DwRequestIdentifierCode` themselves, once `deliverCode` moved outside the ticket's transaction (above): a slow delivery left a window where a duplicate send re-ran `_requestCode`, met the ticket's own resend-delay refusal, and that refusal's `dw_command_outcome` row raced the real answer's — whichever finished writing first stood forever, so a client retrying with the same key could get stuck reading "too many requests" for a code that had, in fact, gone out. `_requestCode` now records the ticket this way, before `deliverCode` runs. If the handler then throws once that row exists: a refusal overwrites it (a later replay answers the refusal, not the stale success); anything else removes it (an incident is never stored under the idempotency key at all, so a later resend runs the handler again — and meets the ticket's own resend-delay refusal, correctly, since the ticket is real either way).
