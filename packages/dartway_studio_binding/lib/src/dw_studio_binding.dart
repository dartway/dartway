...
/// ),
/// ```
class DwStudioBinding extends ConsumerStatefulWidget {
  ...
  /// The app's core. Passed rather than reached ambiently: the binding is
  /// built in `MaterialApp.builder`, where the app's own `dw` is in scope.
  ...
  /// signed in. The framework knows the account; only the project knows which
  /// of its fields means "who this is".
  final ProviderListenable<DwStudioUser?> user;
  ...
  @override
  ConsumerState<DwStudioBinding> createState() => _DwStudioBindingState();
}

class _DwStudioBindingState extends ConsumerState<DwStudioBinding>
  ...
  void initState() {
    ...
    _personaSwitcher = DwStudioPersonaSwitcher(widget.core);
    ...
    ref.listenManual(widget.user, (_, _) {
      _reportSession();
      _reportFeaturesSoon();
    });
  ...
  StudioSessionState _currentSession() {
    ...
    final user = ref.read(widget.user);
    if (user == null) {
  ...
  /// Features are discovered from mounted `DwFeatureWidget` widgets. A route change
